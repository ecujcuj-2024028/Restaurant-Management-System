...
const UserProfile = () => {
  const { profile, loading, error, fetchProfile, updateProfile, updateProfilePicture } =
    useUserStore()
  ...
  if (error)
    return (
      <div className="bg-red-500/10 border border-red-500/30 text-red-400 p-6 rounded-3xl text-center">
        ...
        <button onClick={fetchProfile} className="mt-4 text-sm font-bold underline">
          Reintentar
        </button>
      </div>
    )
      ...
      <div className="lg:col-span-2">
        <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 shadow-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[
              { label: 'Nombre', key: 'name', icon: <UserIcon size={18} />, type: 'text' },
              { label: 'Apellido', key: 'surname', icon: <UserIcon size={18} />, type: 'text' },
            ].map(({ label, key, icon, type }) => (
              <div key={key} className="space-y-2">
                <label className="text-zinc-500 text-[10px] font-bold uppercase tracking-wider ml-1">
                  {label}
                </label>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-600 font-bold text-sm">
                  +502
                </span>
                <input
                  type="text"
          ...
          <div className="mt-10 p-6 bg-orange-500/5 border border-orange-500/10 rounded-2xl flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-zinc-500 text-xs mt-1 leading-relaxed">
                Tu cuenta está protegida con cifrado de extremo a extremo. Para cambiar
                tu contraseña o correo electrónico de seguridad, contacta al administrador
                del sistema.
              </p>
            </div>
          </div>
...
const TABS = [
  ...
  { id: 'role-requests', label: 'Solicitudes de Rol', adminOnly: true },
]

const UserList = () => {
  const { user } = useAuthStore()
  const isAdmin = user?.roles?.includes(ADMIN_SISTEMA)
    ...
    <div className="max-w-5xl mx-auto animate-in fade-in duration-500">
      ...
      <header className="mb-8">
        ...
        <p className="text-zinc-500 mt-1">
          Administra tu perfil y, si eres administrador, las solicitudes de cambio de rol.
        </p>
      </header>

      {/* Pestañas */}
      {visibleTabs.length > 1 && (
        <div className="flex gap-1 mb-8 bg-zinc-900 border border-zinc-800 rounded-2xl p-1 w-fit">
          {visibleTabs.map((tab) => (
            <button
              key={tab.id}
