...
const RestaurantForm = ({ onClose, restaurant }) => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      alert(error?.response?.data?.message || `Error al ${isEditing ? 'actualizar' : 'crear'} el restaurante`)
    }
  }
      ...
      <div className="bg-gray-800 rounded-2xl p-8 w-full max-w-md max-h-screen overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {isSubmitting ? (isEditing ? 'Actualizando...' : 'Creando...') : (isEditing ? 'Guardar Cambios' : 'Crear Restaurante')}
            </button>
          </div>
