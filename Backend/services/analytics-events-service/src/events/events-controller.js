...
const obtenerRolesUsuario = (req) => {
  return Array.isArray(req.userRoles) ? req.userRoles : [];
};

const usuarioEsAdminSistema = (req) => {
  return obtenerRolesUsuario(req).includes(ADMIN_SISTEMA);
};
    ...
    if (data.featuredProducts && data.featuredProducts.length > 0) {
      ...
      if (products.length !== productIds.length) {
        return res.status(404).json({
          success: false,
          message:
            "Uno o más productos no existen o no pertenecen al restaurante indicado",
        });
      }
    }
  ...
  try {
    ...
    if (!product)
      return res.status(404).json({
        success: false,
        message:
          "El producto no existe o no pertenece al restaurante del evento",
      });
